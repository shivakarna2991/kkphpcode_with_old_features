...
class PHPExcel_Writer_HTML extends PHPExcel_Writer_Abstract implements PHPExcel_Writer_IWriter
{
    ...
    public function generateHTMLHeader($pIncludeStyles = false)
    {
		...
		$idaxFont = str_replace(
				"@_IDAX_REPORTS_PATH@",
				_IDAX_REPORTS_PATH,
				file_get_contents(_IDAX_REPORTS_PATH."/templates/html/html_font_include.html")
				);

		$html .= $idaxFont . PHP_EOL;
	...
	private function InPrintRange($printRanges, $col, $row)
	{
		if ($printRanges == null)
		{
			return true;
		...
		foreach ($printRanges as &$printRange)
		{
			if (($col >= $printRange['mincol'])
					&& ($col <= $printRange['maxcol'])
					&& ($row >= $printRange['minrow'])
					&& ($row <= $printRange['maxrow']))
			{
				return true;
			}
		}
    ...
    public function generateSheetData()
    {
        ...
        foreach ($sheets as $sheet) {
			...
			if ($printArea != null)
			{
				$printArea = PHPExcel_Cell::splitRange($printArea);

				$printRanges = array();

				foreach ($printArea as &$area)
				{
					$firstCoord = PHPExcel_Cell::coordinateFromString($area[0]);
					$secondCoord = PHPExcel_Cell::coordinateFromString($area[1]);

					$printRanges[] = array(
							"mincol" => PHPExcel_Cell::columnIndexFromString($firstCoord[0]) - 1,
							"maxcol" => PHPExcel_Cell::columnIndexFromString($secondCoord[0]) - 1,
							"minrow" => $firstCoord[1],
							"maxrow" => $secondCoord[1]
							);
            ...
            while ($row++ < $rowMax) {
                ...
                if (!isset($this->isSpannedRow[$sheet->getParent()->getIndex($sheet)][$row])) {
                    ...
                    while ($column++ < $dimension[1][0]) {
                        // Cell exists?
                        if ($sheet->cellExistsByColumnAndRow($column, $row) && (($printRanges == null) || $this->InPrintRange($printRanges, $column, $row))) {
            ...
            // Write table footer
            $html .= $this->generateTableFooter($sheet);
...
//        $css = $this->mapBorderStyle($pStyle->getBorderStyle()) . ' #' . $pStyle->getColor()->getRGB();
     ...
     * @throws    PHPExcel_Writer_Exception
     */
    private function generateTableFooter($pSheet)
    {
        $html = '    </table>' . PHP_EOL;

		$pHTMLHeaderFooter = $pSheet->getHTMLHeaderFooter();
		$htmlHeader = "";
		...
		if ($pageNumber == 1)
		{
			if ($pHTMLHeaderFooter->getDifferentFirst())
			{
				$htmlFooter = $pHTMLHeaderFooter->getFirstFooter();
			}
			else
			{
				$htmlFooter = $pHTMLHeaderFooter->getOddFooter();
			}
		}
		else
		{
			if (!($pageNumber % 2) && $pHTMLHeaderFooter->getDifferentOddEven())
			{
				$htmlFooter = $pHTMLHeaderFooter->getEvenFooter();
			}
			else
			{
				$htmlFooter = $pHTMLHeaderFooter->getOddFooter();
		...
		// Make substitutions
		$htmlFooter = str_replace("@pageno@", $pageNumber, $htmlFooter);
        ...
        if (is_array($pValues)) {
            ...
            if ($this->isPdf && count($pSheet->getBreaks()) > 0) {
                ...
                if (isset($breaks['A' . $pRow])) {
                    // close table: </table>
                    $html .= $this->generateTableFooter($pSheet);
    ...
    private function setMargins(PHPExcel_Worksheet $pSheet)
    {
		...
		if ($pageNumber == 1)
		{
			if ($pHTMLHeaderFooter->getDifferentFirst())
			{
				$htmlHeader = $pHTMLHeaderFooter->getFirstHeader();
			}
			else
			{
				$htmlHeader = $pHTMLHeaderFooter->getOddHeader();
			}
		}
		else
		{
			if (!($pageNumber % 2) && $pHTMLHeaderFooter->getDifferentOddEven())
			{
				$htmlHeader = $pHTMLHeaderFooter->getEvenHeader();
			}
			else
			{
				$htmlHeader = $pHTMLHeaderFooter->getOddHeader();
		...
		// Make substitutions
		$htmlHeader = str_replace("@pageno@", $pageNumber, $htmlHeader);
