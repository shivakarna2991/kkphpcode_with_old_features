<?php
	...
	class JobSite
	{
		...
		{
//                            echo '<pre>$jobSiteVideoFileRow';print_r($jobSiteVideoFileRow);
//                            echo '<pre>$videoFiles';print_r($videoFiles);
//                            echo '<pre>$dstDirectory';print_r($dstDirectory);
//                            echo '<pre>$dstFilename';print_r($dstFilename);
//                            echo '<pre>$resultString';print_r($resultString);exit;
			DBG_ENTER(DBGZ_VIDEO_JOBSITE, __METHOD__);
			...
			foreach ($videoFiles as &$videoFile)
			{
				...
				exec(
						...
						$status
                                                ...
                                                //echo '<pre>';print_r($ffmpegCmd);exit;
				if (!$status == 0)
				{
			...
			)
		{
			...
			$dstDirectory = "/home/idax/Video/Ingestions/$dstBaseFilename";
                        ...
                        $old_umask = umask(0);
                        //echo 'c'.PROJECT_ROOT."/idax/Video/Ingestions/".$dstBaseFilename;exit;
                       $dstDirectory1= mkdir(PROJECT_ROOT."/idax/Video/Ingestions/".$dstBaseFilename, 0755, true);
                        umask($old_umask);
...
//exit;
			...
			if (!$dstDirectory1)
			//if (!@mkdir(PROJECT_ROOT."/idax/Video/Ingestions/".$dstBaseFilename))
			{
				$result = FALSE;
			...
			if ($result)
			{
				...
				else
				{
					...
					$jobSiteVideoFileRow->CommitChangedFields($sqlError);
                                ...
                                //var_dump($mergePhaseRow->setEndTime(date("Y-m-d H:i:s")));exit;
