	...
	class JobSiteRow
	{
			...
			)
		{
			...
			$retval = FALSE;
                        ...
                        $setupdate = (isset($setupdate) && $setupdate!='') ? $setupdate : '0000-00-00';
                        $oneway = (isset($oneway) && $oneway!='') ? $oneway : '0';
                        //echo '$latitude'.$latitude.'----$longitude'.$longitude.'setupdate'.$setupdate;exit;
			...
			{
				$sqlError = mysqli_errno($dbcon);   /*To Get MySQL Error Number*/
				$sqlError2 = mysqli_error($dbcon);  /*To Get MySQL Exact Error */
                                //echo '<pre>';print_r($sqlError2);exit;
			...
			)
		{
			...
			if ($result)
			{
				...
				for ($i=0; $i<$numRows; $i++)
				{
					...
					if (in_array("DESIGN_STARTED", $new_array))
				  {
				  $rows[$i]['status']="UPLOADED";
				  }
					else if (in_array("DESIGN_COMPLETED", $new_array))
				  {
				  $rows[$i]['status']="READY";
				  }
				 else if (in_array("COUNT_STARTED", $new_array))
				  {
				  $rows[$i]['status']="PROCESSING";
				  }
				else if (in_array("COUNT_PAUSED", $new_array))
				  {
				  $rows[$i]['status']="COUNT_PAUSED";
				  }
				  else if (in_array("QC_STARTED", $new_array))
				  {
				  $rows[$i]['status']="REQUIRES FEEDBACK";
				  }
			...
			else
			{
				...
				$sqlError2 = mysqli_error($dbcon);
                                //echo 'dsdsdsdssss';print_r($sqlError2);exit;
