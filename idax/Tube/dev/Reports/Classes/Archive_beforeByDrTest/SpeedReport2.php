<?php
	...
	class SpeedReport
	{
			...
			)
		{
			...
			foreach ($this->dailyBuckets as $date => &$dailyBucket)
			{
				...
				foreach ($this->hours as &$hour)
				{
					foreach ($this->speedRanges as &$speedRange)
					{
						if (!isset($directionHourlyTotals[$direction][$hour][$speedRange]))
						{
							$directionHourlyTotals[$direction][$hour][$speedRange] = 0;
						}

						$directionHourlyTotals[$direction][$hour][$speedRange] += $dailyBucket['directions'][$direction][$hour][$speedRange];
					}
				}
			}
			...
			)
		{
			...
			foreach ($this->dailyBuckets as $date => &$dailyBucket)
			{
				$this->PopulateOneDayOneDir($date, $this->primaryDirection->GetDr(), $dailyBucket[$this->primaryDirection->GetDr()], $beginningRow);
				$beginningRow += 36;
			}
			
			$this->PopulateAverages('fullday', $this->primaryDirection->GetDr(), $beginningRow);

			foreach ($this->dailyBuckets as $date => &$dailyBucket)
			{
				$this->PopulateOneDayOneDir($date, $this->secondaryDirection->GetDr(), $dailyBucket[$this->secondaryDirection->GetDr()], $beginningRow);
				$beginningRow += 36;
			}
			
			$this->PopulateAverages('fullday', $this->secondaryDirection->GetDr(), $beginningRow);

			$maxRow = $beginningRow + 37;
