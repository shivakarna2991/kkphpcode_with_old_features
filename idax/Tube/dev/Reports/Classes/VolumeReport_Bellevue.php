<?php
	...
	class VolumeReport
	{
			...
			)
		{
			...
			ExcelHelpers::ReplicateRange($this->worksheetTemplates, "G43:H78", $this->multiDayHourlyWorksheet, "A1", TRUE);
			...
			$this->multiDayHourlyWorksheet->setCellValue("B2", $this->location);
			$this->multiDayHourlyWorksheet->mergeCells("B2:F2");

			$this->multiDayHourlyWorksheet->setCellValue("B3", "$startDate - $endDate");
			$this->multiDayHourlyWorksheet->mergeCells("B3:F3");

			$this->multiDayHourlyWorksheet->setCellValue("B4", "$this->siteCode");
			...
			)
		{
			DBG_ENTER(
					...
					__METHOD__,
					"jobId=$jobId, jobSiteId=$jobSiteId, ingestionId=$ingestionId, reportFormat=$reportFormat, startTime=$startTime, endTime=$endTime, siteCode=SsiteCode, primaryDirection=$primaryDirection, secondaryDirection=$secondaryDirection"
					);
			...
			{
				DBG_INFO(DBGZ_TUBE_VOLUMEREPORT, __METHOD__, "Found ".count($this->dataRows)." rows.  Tallying results...");
