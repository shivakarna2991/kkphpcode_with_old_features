			...
			)
		{
			...
			for ($i=0; $i<$countDataPoints; $i++)
			{
				...
				else if ($dataPoints[$i] >= $tenMphRangeEnd)
				{
					...
					// Reset $i to the beginning of the current range.  That's where we
					// we need to start checking data points for the new ten mph range.
					$i = $i - $tenMphCountInRange + 1;

					if ($countDataPoints - $i > $countInRange)
					{
                        ...
                        $i = $i - 2; // So that the beginning of the current range will be visited again when checking the new range, in case it was not captured by the current range

						DBG_INFO_LOWPRIO(DBGZ_TUBE_SPEEDREPORT, __METHOD__, "Starting new range at i=$i, $tenMphRangeStart-$tenMphRangeEnd");
					}
					else
					{
						break;
