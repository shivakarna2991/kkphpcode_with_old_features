<?php
      ...
      )
    {
      ...
      DBG_ENTER(DBGZ_TUBE_SPEEDREPORT, __METHOD__, "count(dataPoints)=$countDataPoints", "speed limie=$speedLimit");
      ...
      {
        DBG_ENTER(DBGZ_TUBE_SPEEDREPORT, __METHOD__, "0 speeding vehicle found");
      }
      else
      {
        $speedingPercentage = $speedingVehicles / $dataPoints;
			...
			)
		{
			...
			foreach ($this->dailyBuckets as $date => &$dailyBucket)
			{
				...
				foreach ($this->hours as &$hour)
				{
					foreach ($this->speedRanges as &$speedRange)
					{
						if (!isset($directionHourlyTotals[$this->primaryDr][$hour][$speedRange]))
						{
							$directionHourlyTotals[$this->primaryDr][$hour][$speedRange] = 0;
							$directionHourlyTotals[$this->secondaryDr][$hour][$speedRange] = 0;
						}

						$directionHourlyTotals[$this->primaryDr][$hour][$speedRange] += $dailyBucket['directions'][$this->primaryDr][$hour][$speedRange];
						$directionHourlyTotals[$this->secondaryDr][$hour][$speedRange] += $dailyBucket['directions'][$this->secondaryDr][$hour][$speedRange];
					}
				}
			}
			...
			)
		{
			...
			foreach ($dailyBucket['directions'] as $direction => &$directionBucket)
			{
				...
				$replicateDestinationRow = $currentRow - 1;
				ExcelHelpers::ReplicateRange($this->worksheetTemplates, "B43:T73", $this->multiDayHourlyWorksheet, "A{$replicateDestinationRow}", TRUE);
				$this->multiDayHourlyWorksheet->setBreak("A{$replicateDestinationRow}" , \PHPExcel_Worksheet::BREAK_ROW);
				...
				$replicationRow = $currentRow - 1;
				ExcelHelpers::ReplicateRange($this->worksheetTemplates, "B75:T79", $this->multiDayHourlyWorksheet, "A{$replicationRow}", TRUE);
				...
				if ($numDataPoints > 0)
				{
					...
					$this->multiDayHourlyWorksheet->setCellValue("J{$currentRow}", "$rangeStart-$rangeEnd");
				...
				else
				{
					...
					$this->multiDayHourlyWorksheet->setCellValue("D{$currentRow}", "0.0");
					$this->multiDayHourlyWorksheet->setCellValue("J{$currentRow}", ".0-10.0");
			...
			)
		{
			...
			ExcelHelpers::ReplicateRange($this->worksheetTemplates, "B2:T42", $this->multiDayHourlyWorksheet, "A1", TRUE);
			...
			// Populate the location, direction, date range, and site code.
      ...
      // Populate speeding fact info
      $currentRow = $beginningRow + 18;
      $postSpeedLimitRow = $currentRow + 1;
      $speedingVehicleRow = $currentRow + 2;
      $speedingPercentageRow = $currentRow + 3;
      $speedingSpeedRow = $currentRow + 4;
      ...
      $this->multiDayHourlyWorksheet->setCellValue("Q{$postSpeedLimitRow}", number_format($this->postSpeedLimit, 1));
      $this->multiDayHourlyWorksheet->setCellValue("Q{$speedingVehicleRow}", $speedingVehicles);
      ...
      //$jobSite = JobSite::FindOne($this->context->dbcon, $jobSiteId, NULL, $sqlError) need confirm by Mike
      //temporary solution:
      $this->stationId = "10";
      $this->specificLocation = "50 Feet E/O W MERCER WAY";
      $this->postSpeedLimit = "30";
      //$jobSite['report parameters'] = "stationid=10001.12;specificlocation=50 feet west from E Mercer Way;postSpeedLimit=30";
