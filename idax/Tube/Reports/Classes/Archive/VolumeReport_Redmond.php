<?php
	...
	class VolumeReport
	{
			...
			)
		{
			...
			ExcelHelpers::ReplicateRange($this->worksheetTemplates, "G43:I78", $this->multiDayHourlyWorksheet, "A1", TRUE);
      ...
      $this->multiDayHourlyWorksheet->setCellValue("C1", $this->location);
			...
			//$this->multiDayHourlyWorksheet->mergeCells("B4:F4");
			$this->multiDayHourlyWorksheet->setCellValue("C4", "$this->stationId");
			$this->multiDayHourlyWorksheet->setCellValue("C5", "$startDate - $endDate");
			//$this->multiDayHourlyWorksheet->mergeCells("B3:F3");
			...
			foreach ($this->hourlyBuckets as $key => &$hourlyBucket)
			{
				...
				if ($currentDayOfWeek != $previousDayOfWeek)
				{
					...
					ExcelHelpers::ReplicateRange($this->worksheetTemplates, "B1:D32", $this->multiDayHourlyWorksheet, "{$dayColumn}4", TRUE);
				...
				//$this->multiDayHourlyWorksheet->getStyle("{spColumn}{$currentDataRow}")->getFont()->getColor()->setARGB(PHPExcel_Style_Color::COLOR_BLACK);
			...
			$this->multiDayHourlyWorksheet->setCellValue("{$col2}8", "$startDate - $endDate");
			...
			)
		{
			...
			ExcelHelpers::ReplicateRange($this->worksheetTemplates, "B41:E78", $this->oneDay1hourWorksheet, "A{$currentRow}", TRUE);
      ...
      $this->speedLimit = "30";
      //$jobSite['report parameters'] = "stationid=10001.12;specificlocation=50 feet west from E Mercer Way;speedlimit=30";
      ...
      //}
			...
			else
			{
				...
				DBG_INFO(DBGZ_TUBE_VOLUMEREPORT, __METHOD__, "numberOfDaysInStudy=$numberOfDaysInStudy, reportStartTime=$reportStartTime, reportEndTime=$reportEndTime, found ".count($this->dataRows)." rows.  Tallying results...");
				...
				else
				{
					...
					$replacements = array(_IDAX_REPORTS_PATH, "Volume", "$primaryDirection / $secondaryDirection", $location, $siteCode, $startDate, $endDate);
