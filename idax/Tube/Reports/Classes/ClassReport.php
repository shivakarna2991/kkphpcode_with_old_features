<?php
	...
	class ClassReport
	{
		...
		private $templateRangeAverage = NULL;
		private $templateRange3DayAverage = NULL;		
		...
		private function PopulateOneDay(
			$date,
			$dailyBucket,
			$beginningRow
			)
		{
			...
			$rowHeights = array(12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 13, 13, 13);
			...
			foreach ($dailyBucket['directions'] as $direction => &$directionBucket)
			{
				...
				{
					$this->multiDayHourlyWorksheet->getRowDimension($currentRow+$i-1)->setRowHeight($rowHeights[$i]);
				}

				$replicateDestinationRow = $currentRow - 1;
				ExcelHelpers::ReplicateRange($this->worksheetTemplates, $this->templateRangeOneDayByHour, $this->multiDayHourlyWorksheet, "A{$replicateDestinationRow}", TRUE);
			...
			)
		{
            ...
            {
                case "standard":
                    $this->templateRangeMultiDaySummary = "E5:S41";
                    $this->templateRangeOneDayByHour = "E44:S74";
                    $this->templateRangeAverage = "E44:S75";
                    $this->templateRange3DayAverage = "E44:S74";
                    break;
                case "bellevue":
                    $this->templateRangeMultiDaySummary = "E5:S41";
                    $this->templateRangeOneDayByHour = "E44:S74";
                    ...
                    break;
                case "redmond":
                    $this->templateRangeMultiDaySummary = "V5:AJ41";
                    $this->templateRangeOneDayByHour = "V44:AJ74";
                    $this->templateRangeAverage = "V44:AJ75";
                    $this->templateRange3DayAverage = "V44:AJ74";
                    break;
            }

			ExcelHelpers::ReplicateRange($this->worksheetTemplates, $this->templateRangeMultiDaySummary, $this->multiDayHourlyWorksheet, "A1", TRUE);

			// Merge a bunch of stuff
			$mergeRanges = array(
					"A1:G2",                                        // Vehicle Classification Report Summary
					"A6:B6", "A7:B7", "A8:B8", "A9:B9", "A10:B10",  // Location, Count Direction, Date Range, Site Code
					"C6:N6", "C7:N7", "C8:N8", "C9:N9", "C10:N10",  // Location, Count Direction, Date Range, Site Code
			...
			)
		{
			...
			foreach ($directionHourlyTotals as $direction => &$directionHourlyTotal)
			{
				...
				if ($type == 'fullday')
				{
					$templateRange = $this->templateRangeAverage;
					$header = "Total Study Average";
				}
				else if ($type == 'midweek')
				{
					$templateRange = $this->templateRange3DayAverage;
                ...
                // Merge the note row
				$currentRow += 2;
				$this->multiDayHourlyWorksheet->mergeCells("A{$currentRow}:N{$currentRow}");

				// Skip over unused/empty rows to the next direction
				$currentRow += ($type == 'fullday') ? 2 : 1;
			}
			...
			)
		{
			...
			$rowHeights = array(17, 17, 17, 17, 12, 12, 12, 12, 12, 12, 12, 13, 12, 13, 13, 12, 14, 12, 14, 12, 15, 12, 12, 13, 12, 12, 12, 12, 12, 12, 12, 13, 13, 13, 13, 13, 13);
			...
			foreach ($this->dailyBuckets as $date => &$dailyBucket)
			{
				$this->PopulateOneDay($date, $dailyBucket, $beginningRow);
				$beginningRow += 62;
			}
			...
			)
		{
			DBG_ENTER(
					...
					__METHOD__,
					"jobSiteId=$jobSiteId, ingestionId=$ingestionId, reportFormat=$reportFormat, reportParameters=$reportParameters, startTime=$startTime, endTime=$endTime, siteCode=SsiteCode, primaryDirection=$primaryDirection, secondaryDirection=$secondaryDirection"
					);
			...
			{				
				...
				DBG_INFO(
                        DBGZ_TUBE_CLASSREPORT,
                        __METHOD__,
                        "office id = $office, stationid=$this->stationId, specificlocation=$this->specificLocation, speedlimit=$this->postSpeedLimit"
                        );
			...
			else
			{
				...
				else
				{
					...
					$replacements = array(_IDAX_REPORTS_PATH, "Class", "$primaryDirection / $secondaryDirection", $location, $siteCode, $startDate, $endDate, $this->stationId, $this->specificLocation, $officeEmail);
					...
					$pageSetup->setPrintArea('A11:O32,A39:O'.$maxRow);
