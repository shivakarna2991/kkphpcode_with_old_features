...
class AWSFileManager {
    ...
    $destination, $root = false, $key = false,$secret = false
        ...
        if ($root) {
            ...
            $this->root = PROJECT_ROOT . DIRECTORY_SEPARATOR;
        }
    ...
    $sourceFilename, $destinationFilename, $cannedacl, $overwrite, &$resultString
        ...
        if ($filecontents === FALSE) {
            ...
            $result = $this->UploadData($destinationFilename,$sourceFilename, $cannedacl, $filecontents, $mimetype, $overwrite, $resultString);
        }
    ...
    public function UploadData(
    $name, $sourceFilename, $cannedacl, $contents, $contenttype, $overwrite, &$resultString
    ) {
		...
		$this->file['filename']	= $this->filename;*/
             //echo 'dsdsds';exit;
		//set full path
		//echo $this->file['full_path'] = $this->root . $this->destination . $this->filename;exit;
                echo $sourceFilename;
		$this->file['full_path'] = $this->root . $this->destination . $name;
        	$this->file['path'] = $this->destination . $name;
                if(copy($sourceFilename, $this->file['path'])){
                    ...
                    //echo 'error';
                    $resultString = "file not transferred successfully";
                }
                //exit;
...
//                    $resultString = "Upload: Can\'t upload file. - statusCode=$status";
//                        DBG_ERR(DBGZ_AWSFILEMGR, __METHOD__, $resultString);
//		}
        ...
        //echo '<pre>345';print_r($result);exit;
        DBG_RETURN_BOOL(DBGZ_AWSFILEMGR, __METHOD__, $result);
        return $result;
    }
